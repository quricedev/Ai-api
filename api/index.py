...
def ai_api():
    ...
    if not key or not prompt:
        return jsonify({"error": "Missing parameters. DM @UseSir / @OverShade"}), 400

    doc = keys_col.find_one({"key": key, "active": True})
    if not doc:
        return jsonify({"error": "Invalid API key. DM @UseSir / @OverShade"}), 401

    if doc["expires_at"] < datetime.datetime.utcnow():
        keys_col.update_one({"key": key}, {"$set": {"active": False}})
        return jsonify({"error": "API key expired. DM @UseSir / @OverShade"}), 401

    reply, latency = call_ai(prompt)
    keys_col.update_one({"key": key}, {"$inc": {"usage": 1}})

    return jsonify({
        "provider": "@UseSir / @OverShade",
        "reply": reply,
...
if bot:
    ...
    def test_cmd(m):
        ...
        if target.lower() == "main":
            ...
            return
        doc = keys_col.find_one({"key": target}) or keys_col.find_one({"name": target})
        if not doc:
            bot.send_message(m.chat.id, "*Key not found*", parse_mode="Markdown")
            return
        _, latency = call_ai("OK")
